...
if ($data != []) {
    ...
    if ($data['user_id'] != '') {
        ...
        if ($data['survey_id'] != '') {
            ...
            if ($data['survey_name'] != '') {
                $survey_name = htmlentities(strip_tags($data['survey_name']), ENT_QUOTES, 'UTF-8');
                ...
                if ($data['survey_results'] != '') {
                    ...
                    $sql = "INSERT INTO surveyresults1 (user_id, survey_id, survey_name, json, country, state, dob, gender) VALUES ('$user_id', '$survey_id', '$survey_name', '$survey_results', '$cc', '$ss', '$db', '$gd')";
                    if ($conn->query($sql)) {
                ...
                echo json_encode(array(
                    "status" => "Error",
                    "message" => "Survey Common Name is Required",
                ));
            }
